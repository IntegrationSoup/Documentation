# Workflow

This section covers how to build workflows in Integration Soup Integration Host.

## Core Concepts

- [Variables](variables.md)
- [Paths](paths.md)
- [Paths For AI](reference/pathsforai.md)
- [Paths Detailed](reference/pathsindepth.md)

## Activity Guides

- [Receiver Activities](receiver-activities/index.md)
- [Sender Activities](sender-activities/index.md)
- [Extension Libraries](extension-libraries/index.md)

## API and JSON Contracts

- [Workflow API](api/index.md)
- [Workflow Reference](reference/index.md)
- [Workflow Enum and Interface Reference](reference/workflow-enums-and-interfaces.md)
- [Filter Host (FilterHostSetting)](reference/filter-host-setting.md)
- [Transformer Setting (TransformerSetting)](reference/transformer-setting.md)
- [Variable Creator JSON Reference](reference/variable-creator.md)
- [MessageTypeOptions JSON Reference](reference/message-type-options.md)

## AI Authoring

- [AI Workflow Docs](ai/index.md)

## Examples

- [Examples](examples/index.md)
- [SenderExample.hl7workflow](examples/SenderExample.hl7workflow)
- [SenderExamplePart2.hl7workflow](examples/SenderExamplePart2.hl7workflow)

## Navigation

- [Documentation Sitemap](../sitemap.md)
